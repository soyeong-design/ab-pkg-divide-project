'use client'

import * as Dialog from '@radix-ui/react-dialog'
import { Button } from '@/components/ui/Button'
import { cn } from '@/lib/cn'

interface ConfirmResetDialogProps {
  onCancel: () => void
  onConfirm: () => void
}

export function ConfirmResetDialog({ onCancel, onConfirm }: ConfirmResetDialogProps) {
  return (
    <Dialog.Root open onOpenChange={(o) => !o && onCancel()}>
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 bg-black/40 z-50 transition-opacity" />
        <Dialog.Content
          className={cn(
            'fixed left-[50%] top-[50%] z-50 w-[400px] translate-x-[-50%] translate-y-[-50%]',
            'bg-white rounded-[20px] shadow-[0_4px_24px_rgba(0,0,0,0.08)] overflow-hidden',
            'flex flex-col'
          )}
        >
          {/* Header */}
          <div className="flex h-16 items-center px-6 border-b border-[#dee2e6]">
            <Dialog.Title className="text-[18px] font-bold text-[#212529] tracking-[-0.3px]">
              수정된 수량을 원상복구 하겠습니까?
            </Dialog.Title>
            <button onClick={onCancel} className="ml-auto flex items-center justify-center p-2 rounded-full hover:bg-gray-100 transition-colors">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                <path d="M18 6L6 18M6 6L18 18" stroke="#868e96" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
            </button>
          </div>

          {/* Body */}
          <div className="p-6">
            <p className="text-[15px] leading-6 text-[#495057] tracking-[-0.3px]">
              전체 상품 포장을 누를 경우, 입력하신 내용이 <strong className="text-[#008fff]">삭제</strong>되며<br/>
              주문 전체 수량으로 설정됩니다.<br/>
              계속하시겠습니까?
            </p>
          </div>

          {/* Footer */}
          <div className="flex items-center gap-2 px-6 pb-6 pt-2">
            <Button
              variant="outline"
              className="flex-1 h-12 text-[15px] font-semibold tracking-[-0.3px] border-[#adb5bd] text-[#495057]"
              onClick={onCancel}
            >
              취소
            </Button>
            <Button
              className="flex-1 h-12 bg-black hover:bg-black/90 text-[15px] font-semibold tracking-[-0.3px] text-white"
              onClick={onConfirm}
            >
              확인
            </Button>
          </div>
        </Dialog.Content>
      </Dialog.Portal>
    </Dialog.Root>
  )
}
